    ...
    return (
        <div className='h-full relative flex justify-center items-center'>
            <AnimatePresence mode='wait'>
                {title && (
                    <motion.div key={title} className='absolute top-14 md:top-32 overflow-hidden z-20'>
            ...
            <div className='overflow-hidden flex gap-4'>
                {data.map((item, idx) => (
                    <motion.img
                        ...
                        className={`object-cover w-[40vw] md:w-[200px] h-[300px] ${idx === 0 || idx === 1 ? '' : 'hidden md:inline'}`}
                        key={item.title}
                        src={item.url}
                        alt={item.title}
                    />
